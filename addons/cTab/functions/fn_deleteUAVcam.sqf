...
_camToDelete = if (count _this isEqualTo 1) then {_this select 0} else {objNull};

// remove cameras
for "_i" from (count cTabUAVcams -1) to 0 step -1 do {
	_cam = cTabUAVcams select _i select 2;
	if (isNull _camToDelete || {_cam isEqualTo _camToDelete}) then {
...
if (count cTabUAVcams isEqualTo 0) then {
