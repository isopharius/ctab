...
call {
	...
	if (isServer) exitWith {
		if (isNil "_transactionId") then {
			...
			if (_markerData select 1 isEqualTo 20) then { _markerData set [4, "VRP"]; };
			if (_markerData select 1 isEqualTo 21) then { _markerData set [4, "CCP"]; };
			if (_markerData select 1 isEqualTo 22) then { _markerData set [4, "BAS"]; };
			if (_markerData select 1 isEqualTo 23) then { _markerData set [4, "HQ"]; };

			// Increment marker label.
			if (_markerData select 1 isEqualTo 30) then {
				cTab_markerCounterMKR = cTab_markerCounterMKR + 1;
				_markerData set [4, format ["%1", cTab_markerCounterMKR]];
			};

			// Increment waypoint label.
			if (_markerData select 1 isEqualTo 31) then {
			...
			if (_markerData select 1 isEqualTo 32) then {
			...
			if (_markerData select 1 isEqualTo 33) then {
			...
			if (_markerData select 1 isEqualTo 34) then {
